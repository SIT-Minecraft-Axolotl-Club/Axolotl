...
- **受管实例不能被删除**:`theseus::instance::remove` 会先查 `managed_instances`,命中就直接报错 `Instance … is managed by the society server as "…" and cannot be deleted`。界面同步收敛:实例右键菜单、批量删除、实例设置里的「删除实例」入口对受管实例一律隐藏(`isManagedInstance()`,数据来自 `managed_list`),批量删除只删可删的那些;即使集合过期,删除请求也会被上面的实例层拒绝,不会真的删掉。要让玩家看不到某个实例,应把它从后端清单里移除(客户端会标记为 `retired`,数据保留但不可启动)。
...
`required` 字段承载"是否强制下发",后台那一栏现在就叫「**强制下发**」:

- **强制**(`required: true`):同步时自动下载并保持最新;玩家不能删除(实例层拒绝,菜单也不提供);必须装到当前 revision 才能启动。
- **非强制**(`required: false`):**不自动下载**——实例照常出现在首页面板并标注「未下载」,玩家点「开始游戏(需先下载)」时才下载并接着启动;**允许玩家删除**(`instance::remove` 只对强制实例报错),删除只释放磁盘,记录保留,因此实例仍在列表里,随时可以再下载。
- **服务端移除实例**:同步不再把它标成 `retired` 留着,而是**删除本地实例、文件与记录**,并派发 `Removed` 事件让界面即时消失。

这样社团可以把"必须所有人都装"的整合包标为强制,把可选/体积大的整合包标为非强制,不再无条件占用每个玩家的磁盘。
...
| `packages/app-lib/migrations/20260918000000_default-download-sources-to-bmclapi.sql` | 上游默认下载来源是 `auto`(按系统时区/语言判断是否走镜像) | 社团默认改为 `mirror_preferred`(优先 OpenBMCLAPI,官方源保留为后备),覆盖 Minecraft 元数据、游戏文件、模组加载器与 Java;玩家手动选过的来源不会被覆盖 |
