//! Hosting for the club's map pages.
//!
//! The club's map servers answer with `X-Frame-Options: SAMEORIGIN`, so the
//! pages can never render inside an `<iframe>` on the launcher page and the
//! club decided not to change the servers. A child webview is a top-level
...
/// Label of the single webview that hosts the club maps. It is stable so the
/// frontend can open, resize, navigate and close it without holding a handle.
const CLUB_MAP_WEBVIEW_LABEL: &str = "club-map";
...
fn parse_remote_url(url: &str) -> Result<tauri::Url, String> {
    let parsed = url
        .parse::<tauri::Url>()
        .map_err(|error| format!("Invalid club map URL {url}: {error}"))?;
    if !matches!(parsed.scheme(), "http" | "https") {
        return Err(format!(
            "Refusing to host club map URL {url}: only http and https pages are supported"
        ));
    }
    Ok(parsed)
}
...
fn club_map_webview(window: &Window) -> Option<Webview> {
    window
        .webviews()
        .into_iter()
        .find(|webview| webview.label() == CLUB_MAP_WEBVIEW_LABEL)
}

/// Creates the club map webview if needed, then shows `url` at the requested
...
pub fn map_webview_open(
    ...
    let webview = match club_map_webview(&window) {
        Some(webview) => webview,
        None => window
            .add_child(
                WebviewBuilder::new(
                    CLUB_MAP_WEBVIEW_LABEL,
            ...
            .map_err(|error| {
                format!("Failed to create the club map webview: {error}")
            })?,
    };

    webview.set_position(position).map_err(|error| {
        format!("Failed to move the club map webview: {error}")
    })?;
    webview.set_size(size).map_err(|error| {
        format!("Failed to resize the club map webview: {error}")
    })?;

    let target = url.to_string();
    webview.navigate(url).map_err(|error| {
        format!("Failed to load the club map page {target}: {error}")
    })?;
    webview.show().map_err(|error| {
        format!("Failed to show the club map webview: {error}")
    })
}

/// Moves and resizes the club map webview. Called for every layout change the
...
pub fn map_webview_set_bounds(
    ...
    let Some(webview) = club_map_webview(&window) else {
        return Ok(());
    };
    let (position, size) = bounds(x, y, width, height);
...
/// Navigates the club map webview to another map. The frontend always opens a
...
/// Destroys the club map webview so a page that is left behind cannot keep
...
pub fn map_webview_close(window: Window) -> Result<(), String> {
    ...
    webview.close().map_err(|error| {
        format!("Failed to close the club map webview: {error}")
    })
}
