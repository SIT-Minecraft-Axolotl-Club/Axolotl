...
| `required` | boolean | 否（默认 `true`） | **强制下发**开关。`true`：客户端在同步时自动下载/更新到清单指定的 `revision`，玩家不能删除该实例，且必须装到当前版本才允许启动。`false`：客户端**不**自动下载——实例仍会显示在列表里并标注"未下载"，只有玩家点「开始游戏」时才下载并随后启动;玩家可以自行删除以释放磁盘,删除后该实例仍留在列表中可随时重新下载。服务端把实例从 `instances` 数组里移除时,客户端会**连同本地实例与记录一起删除** |
